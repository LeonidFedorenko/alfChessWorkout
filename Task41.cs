...
namespace alfChessWorkout
{
    ...
    {   // картинка из PNG (все фигуры)
        ...
        {   // форма для 41 этюда
            ...
            this.Text = " Этюд №41. Мат в один ход. Ход черных.";
            // загрузка фигур для этюда №41
            Init();
        }

        public void showTask41_click(object sender, EventArgs e)
        {
            if (pictureBox1.Visible == true)
            {
                pictureBox1.Visible = false; // скрываю рисунок (с задачей)
                buttonTask41.Text = "   ОБНОВИТЬ       ДАННЫЙ   ЭТЮД";
            }
            else
            {   // перезапуск этюда
                this.Controls.Clear();
        ...
        {   // расстановка фигур по умолчанию (для перезапуска партии)
            ...
            currentPlayer = 2; // текущий игрок по умолчанию (черные)
        ...
        {   // кнопка не нажата - фон стандартный
            ...
            if ((userSteps == 1) && (pressedButton.Location.X == 484) && (pressedButton.Location.Y == 298) && (chessboard[0, 7] == 0))
            {
                labelTask41.ForeColor = Color.GreenYellow;
                labelTask41.Text = "Вы успешно решили Этюд №41.";
                MessageBox.Show("Вы успешно решили Этюд №41.", "Обновить Этюд №41", MessageBoxButtons.OK, MessageBoxIcon.Information);
            ...
            {
                labelTask41.ForeColor = Color.OrangeRed;
                labelTask41.Text = "Решить за 1 ход не удалось.";
            }
