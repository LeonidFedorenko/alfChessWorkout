...
namespace alfChessWorkout
{
    ...
    {   // картинка из PNG (все фигуры)
        ...
        {   // форма для 40 этюда
            ...
            this.Text = " Этюд №40. Мат в один ход. Ход черных.";
            // загрузка фигур для этюда №40
            Init();
        }

        public void showTask40_click(object sender, EventArgs e)
        {
            if (pictureBox1.Visible == true)
            {
                pictureBox1.Visible = false; // скрываю рисунок (с задачей)
                buttonTask40.Text = "   ОБНОВИТЬ       ДАННЫЙ   ЭТЮД";
            }
            else
            {   // перезапуск этюда
                this.Controls.Clear();
        ...
        {   // расстановка фигур по умолчанию (для перезапуска партии)
            ...
            currentPlayer = 1; // текущий игрок по умолчанию (белый)
        ...
        {   // кнопка не нажата - фон стандартный
            ...
            if ((userSteps == 1) && (pressedButton.Location.X == 236) && (pressedButton.Location.Y == 484)) 
            {
                labelTask40.ForeColor = Color.GreenYellow;
                labelTask40.Text = "Вы успешно решили Этюд №40.";
                MessageBox.Show("Вы успешно решили Этюд №40.", "Обновить Этюд №40", MessageBoxButtons.OK, MessageBoxIcon.Information);
            ...
            {
                labelTask40.ForeColor = Color.OrangeRed;
                labelTask40.Text = "Решить за 1 ход не удалось.";
            }
