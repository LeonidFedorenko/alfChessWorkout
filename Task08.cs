...
namespace alfChessWorkout
{
    ...
    {   // картинка из PNG (все фигуры)
        ...
        {   // 1 - белые, 2 - черные
            ...
            {25,24,0,22,0,23,24,25},
            {26,26,26,0,21,13,26,0},
            {0,0,0,26,0,0,0,26},
            {0,0,0,0,14,0,0,0},
            {0,0,0,0,16,0,0,0},
            {0,0,14,0,0,0,0,0},
            {16,16,16,16,0,16,16,16},
            {15,0,13,23,11,0,0,15},
        };
        ...
        {   // форма для 8 этюда
            ...
            this.Text = " Этюд №8. Мат в один ход. Ход белых.";
            // загрузка фигур для этюда №8
            Init();
        }

        public void showTask08_click(object sender, EventArgs e)
        {
            if (pictureBox1.Visible == true)
            {
                pictureBox1.Visible = false; // скрываю рисунок (с задачей)
                buttonTask08.Text = "   ОБНОВИТЬ       ДАННЫЙ   ЭТЮД";
            }
            else
            {   // перезапуск этюда
                this.Controls.Clear();
        ...
        {   // создаю доску для игры (8 x 8)
            for (int iChess = 0; iChess < 8; iChess++)
            {
                ...
                {   // массив возможных ходов для каждой фигуры
                    ...
                    buttonF.BackColor = Color.FromArgb(124, 149, 94); // темный фон кнопки-фигуры
                    if ((iChess + jChess) % 2 == 0)
                    {
                        buttonF.BackColor = Color.FromArgb(238, 238, 214); // светлый фон кнопки-фигуры    
                    }
        ...
        {   // кнопка не нажата - фон стандартный
            ...
            {
                if (previousButton.BackColor == Color.FromArgb(238, 238, 214))
                {   // был светлый - оставляю
                    previousButton.BackColor = Color.FromArgb(238, 238, 214);
                }
                else if (previousButton.BackColor == Color.FromArgb(124, 149, 94))
                {   // был тёмный - оставляю
                    previousButton.BackColor = Color.FromArgb(124, 149, 94);
                }
            }
            ...
            if ((userSteps == 1) && (pressedButton.Location.X == 236) && (pressedButton.Location.Y == 236) && (chessboard[5, 2] == 0))
            {
                labelTask08.ForeColor = Color.Yellow;
                labelTask08.Text = "Вы успешно решили Этюд №8.";
                MessageBox.Show("Вы успешно решили Этюд №8.", "Обновить Этюд №8", MessageBoxButtons.OK, MessageBoxIcon.Information);
            ...
            {
                labelTask08.ForeColor = Color.Cyan;
                labelTask08.Text = "Решить за 1 ход не удалось.";
            }
        ...
        {   // убираю вспомогательные шаги-подсказки
            for (int iStep = 0; iStep < 8; iStep++)
            {
                for (int jStep = 0; jStep < 8; jStep++)
                {
                    ...
                    {
                        virtualButtons[iStep, jStep].BackColor = Color.FromArgb(238, 238, 214);
                    }
                    else if ((iStep % 2 == 1) && ((jStep == 0) || (jStep == 2) || (jStep == 4) || (jStep == 6) || (jStep == 8)))
                    {
                        virtualButtons[iStep, jStep].BackColor = Color.FromArgb(124, 149, 94);
                    }
