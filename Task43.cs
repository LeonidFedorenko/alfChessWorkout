...
namespace alfChessWorkout
{
    ...
    {   // картинка из PNG (все фигуры)
        ...
        {   // форма для 43 этюда
            ...
            this.Text = " Этюд №43. Мат в один ход. Ход белых.";
            // загрузка фигур для этюда №43
            Init();
        }

        public void showTask43_click(object sender, EventArgs e)
        {
            if (pictureBox1.Visible == true)
            {
                pictureBox1.Visible = false; // скрываю рисунок (с задачей)
                buttonTask43.Text = "   ОБНОВИТЬ       ДАННЫЙ   ЭТЮД";
            }
            else
            {   // перезапуск этюда
                this.Controls.Clear();
        ...
        {   // кнопка не нажата - фон стандартный
            ...
            if ((userSteps == 1) && (pressedButton.Location.X == 484) && (pressedButton.Location.Y == 236) && (chessboard[3, 0] == 0))
            {
                labelTask43.ForeColor = Color.GreenYellow;
                labelTask43.Text = "Вы успешно решили Этюд №43.";
                MessageBox.Show("Вы успешно решили Этюд №43.", "Обновить Этюд №43", MessageBoxButtons.OK, MessageBoxIcon.Information);
            ...
            {
                labelTask43.ForeColor = Color.OrangeRed;
                labelTask43.Text = "Решить за 1 ход не удалось.";
            }
