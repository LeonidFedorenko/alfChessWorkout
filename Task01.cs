...
namespace alfChessWorkout
{
    ...
    {   // картинка из PNG (все фигуры)
        ...
        {   // форма для 1 этюда
            ...
            this.Text = " Этюд №1. Мат в один ход. Ход белых.";
            // загрузка фигур для этюда №1
            Init();
        }

        public void showTask01_click(object sender, EventArgs e)
        {
            if (pictureBox1.Visible == true)
            {
                pictureBox1.Visible = false; // скрываю рисунок (с задачей)
                buttonTask01.Text = "   ОБНОВИТЬ       ДАННЫЙ   ЭТЮД";
            }
            else
            {   // перезапуск этюда
                this.Controls.Clear();
        ...
        {   // кнопка не нажата - фон стандартный
            ...
            if ((userSteps == 1) && (pressedButton.Location.X == 298) && (pressedButton.Location.Y == 236) && (chessboard[7, 0] == 0))
            {
                labelTask01.ForeColor = Color.GreenYellow;
                labelTask01.Text = "Вы успешно решили Этюд №1.";
                MessageBox.Show("Вы успешно решили Этюд №1.", "Обновить Этюд №1", MessageBoxButtons.OK, MessageBoxIcon.Information);
            ...
            {
                labelTask01.ForeColor = Color.OrangeRed;
                labelTask01.Text = "Решить за 1 ход не удалось.";
            }
