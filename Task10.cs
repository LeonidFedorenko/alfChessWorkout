...
namespace alfChessWorkout
{
    ...
    {   // картинка из PNG (все фигуры)
        ...
        {   // форма для 1 этюда
            ...
            this.Text = " Этюд №10. Мат в один ход. Ход белых.";
            // загрузка фигур для этюда №10
            Init();
        }

        public void showTask10_click(object sender, EventArgs e)
        {
            if (pictureBox1.Visible == true)
            {
                pictureBox1.Visible = false; // скрываю рисунок (с задачей)
                buttonTask10.Text = "   ОБНОВИТЬ       ДАННЫЙ   ЭТЮД";
            }
            else
            {   // перезапуск этюда
                this.Controls.Clear();
        ...
        {   // кнопка не нажата - фон стандартный
            ...
            if ((userSteps == 1) && (pressedButton.Location.X == 422) && (pressedButton.Location.Y == 236) && (chessboard[2, 5] == 0))
            {
                labelTask10.ForeColor = Color.GreenYellow;
                labelTask10.Text = "Вы успешно решили Этюд №10.";
                MessageBox.Show("Вы успешно решили Этюд №10.", "Обновить Этюд №10", MessageBoxButtons.OK, MessageBoxIcon.Information);
            ...
            {
                labelTask10.ForeColor = Color.OrangeRed;
                labelTask10.Text = "Решить за 1 ход не удалось.";
            }
