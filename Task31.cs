...
namespace alfChessWorkout
{
    ...
    {   // картинка из PNG (все фигуры)
        ...
        {   // форма для 31 этюда
            ...
            this.Text = " Этюд №31. Мат в один ход. Ход белых.";
            // загрузка фигур для этюда №31
            Init();
        }

        public void showTask31_click(object sender, EventArgs e)
        {
            if (pictureBox1.Visible == true)
            {
                pictureBox1.Visible = false; // скрываю рисунок (с задачей)
                buttonTask31.Text = "   ОБНОВИТЬ       ДАННЫЙ   ЭТЮД";
            }
            else
            {   // перезапуск этюда
                this.Controls.Clear();
        ...
        {   // кнопка не нажата - фон стандартный
            ...
            if ((userSteps == 1) && (pressedButton.Location.X == 484) && (pressedButton.Location.Y == 112) && (chessboard[2, 6] == 0))
            {
                labelTask31.ForeColor = Color.GreenYellow;
                labelTask31.Text = "Вы успешно решили Этюд №31.";
                MessageBox.Show("Вы успешно решили Этюд №31.", "Обновить Этюд №31", MessageBoxButtons.OK, MessageBoxIcon.Information);
            ...
            {
                labelTask31.ForeColor = Color.OrangeRed;
                labelTask31.Text = "Решить за 1 ход не удалось.";
            }
