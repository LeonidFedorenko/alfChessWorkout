...
namespace alfChessWorkout
{
    ...
    {   // картинка из PNG (все фигуры)
        ...
        {   // форма для 15 этюда
            ...
            this.Text = " Этюд №15. Мат в один ход. Ход белых.";
            // загрузка фигур для этюда №15
            Init();
        }

        public void showTask15_click(object sender, EventArgs e)
        {
            if (pictureBox1.Visible == true)
            {
                pictureBox1.Visible = false; // скрываю рисунок (с задачей)
                buttonTask15.Text = "   ОБНОВИТЬ       ДАННЫЙ   ЭТЮД";
            }
            else
            {   // перезапуск этюда
                this.Controls.Clear();
        ...
        {   // кнопка не нажата - фон стандартный
            ...
            if ((userSteps == 1) && (pressedButton.Location.X == 112) && (pressedButton.Location.Y == 236) && (chessboard[6, 4] == 0))
            {
                labelTask15.ForeColor = Color.GreenYellow;
                labelTask15.Text = "Вы успешно решили Этюд №15.";
                MessageBox.Show("Вы успешно решили Этюд №15.", "Обновить Этюд №15", MessageBoxButtons.OK, MessageBoxIcon.Information);
            ...
            {
                labelTask15.ForeColor = Color.OrangeRed;
                labelTask15.Text = "Решить за 1 ход не удалось.";
            }
