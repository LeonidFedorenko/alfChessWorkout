...
namespace alfChessWorkout
{
    ...
    {   // картинка из PNG (все фигуры)
        ...
        {   // форма для 1 этюда
            ...
            this.Text = " Этюд №5. Мат в один ход. Ход белых.";
            // загрузка фигур для этюда №1
            Init();
        }

        public void showTask05_click(object sender, EventArgs e)
        {
            if (pictureBox1.Visible == true)
            {
                pictureBox1.Visible = false; // скрываю рисунок (с задачей)
                buttonTask05.Text = "   ОБНОВИТЬ       ДАННЫЙ   ЭТЮД";
            }
            else
            {   // перезапуск этюда
                this.Controls.Clear();
        ...
        {   // кнопка не нажата - фон стандартный
            ...
            if ((userSteps == 1) && (pressedButton.Location.X == 422) && (pressedButton.Location.Y == 298) && (chessboard[4, 3] == 0))
            {
                labelTask05.ForeColor = Color.GreenYellow;
                labelTask05.Text = "Вы успешно решили Этюд №5.";
                MessageBox.Show("Вы успешно решили Этюд №5.", "Обновить Этюд №5", MessageBoxButtons.OK, MessageBoxIcon.Information);
            ...
            {
                labelTask05.ForeColor = Color.OrangeRed;
                labelTask05.Text = "Решить за 1 ход не удалось.";
            }
