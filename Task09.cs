...
namespace alfChessWorkout
{
    ...
    {   // картинка из PNG (все фигуры)
        ...
        {   // форма для 9 этюда
            ...
            this.Text = " Этюд №9. Мат в один ход. Ход белых.";
            // загрузка фигур для этюда №1
            Init();
        }

        public void showTask09_click(object sender, EventArgs e)
        {
            if (pictureBox1.Visible == true)
            {
                pictureBox1.Visible = false; // скрываю рисунок (с задачей)
                buttonTask09.Text = "   ОБНОВИТЬ       ДАННЫЙ   ЭТЮД";
            }
            else
            {   // перезапуск этюда
                this.Controls.Clear();
        ...
        {   // создаю доску для игры (8 x 8)
            for (int iChess = 0; iChess < 8; iChess++)
            {
                ...
                {   // массив возможных ходов для каждой фигуры
                    ...
                    buttonF.BackColor = Color.FromArgb(201, 148, 98); // темный фон кнопки-фигуры
                    if ((iChess + jChess) % 2 == 0)
                    {
                        buttonF.BackColor = Color.FromArgb(255, 255, 255); // светлый фон кнопки-фигуры    
                    }
        ...
        {   // кнопка не нажата - фон стандартный
            ...
            {
                if (previousButton.BackColor == Color.FromArgb(255, 255, 255))
                {   // был светлый - оставляю
                    previousButton.BackColor = Color.FromArgb(255, 255, 255);
                }
                else if (previousButton.BackColor == Color.FromArgb(201, 148, 98))
                {   // был тёмный - оставляю
                    previousButton.BackColor = Color.FromArgb(201, 148, 98);
                }
            }
            ...
            if ((userSteps == 1) && (pressedButton.Location.X == 360) && (pressedButton.Location.Y == 112) && (chessboard[2, 5] == 0))
            {
                labelTask09.ForeColor = Color.GreenYellow;
                labelTask09.Text = "Вы успешно решили Этюд №9.";
                MessageBox.Show("Вы успешно решили Этюд №9.", "Обновить Этюд №9", MessageBoxButtons.OK, MessageBoxIcon.Information);
            ...
            {
                labelTask09.ForeColor = Color.OrangeRed;
                labelTask09.Text = "Решить за 1 ход не удалось.";
            }
        ...
        {   // убираю вспомогательные шаги-подсказки
            for (int iStep = 0; iStep < 8; iStep++)
            {
                for (int jStep = 0; jStep < 8; jStep++)
                {
                    ...
                    {
                        virtualButtons[iStep, jStep].BackColor = Color.FromArgb(255, 255, 255);
                    }
                    else if ((iStep % 2 == 1) && ((jStep == 0) || (jStep == 2) || (jStep == 4) || (jStep == 6) || (jStep == 8)))
                    {
                        virtualButtons[iStep, jStep].BackColor = Color.FromArgb(201, 148, 98);
                    }
