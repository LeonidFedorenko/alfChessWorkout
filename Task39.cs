...
namespace alfChessWorkout
{
    ...
    {   // картинка из PNG (все фигуры)
        ...
        {   // форма для 39 этюда
            ...
            this.Text = " Этюд №39. Мат в один ход. Ход белых.";
            // загрузка фигур для этюда №39
            Init();
        }

        public void showTask39_click(object sender, EventArgs e)
        {
            if (pictureBox1.Visible == true)
            {
                pictureBox1.Visible = false; // скрываю рисунок (с задачей)
                buttonTask39.Text = "   ОБНОВИТЬ       ДАННЫЙ   ЭТЮД";
            }
            else
            {   // перезапуск этюда
                this.Controls.Clear();
        ...
        {   // кнопка не нажата - фон стандартный
            ...
            if ((userSteps == 1) && (pressedButton.Location.X == 422) && (pressedButton.Location.Y == 112) && (chessboard[4,2]==0))
            {
                labelTask39.ForeColor = Color.GreenYellow;
                labelTask39.Text = "Вы успешно решили Этюд №39.";
                MessageBox.Show("Вы успешно решили Этюд №39.", "Обновить Этюд №39", MessageBoxButtons.OK, MessageBoxIcon.Information);
            ...
            {
                labelTask39.ForeColor = Color.WhiteSmoke;
                labelTask39.Text = "Решить за 1 ход не удалось.";
            }
