...
namespace alfChessWorkout
{
    ...
    {   // картинка из PNG (все фигуры)
        ...
        {   // форма для 27 этюда
            ...
            this.Text = " Этюд №27. Мат в один ход. Ход белых.";
            // загрузка фигур для этюда №27
            Init();
        }

        public void showTask27_click(object sender, EventArgs e)
        {
            if (pictureBox1.Visible == true)
            {
                pictureBox1.Visible = false; // скрываю рисунок (с задачей)
                buttonTask27.Text = "   ОБНОВИТЬ       ДАННЫЙ   ЭТЮД";
            }
            else
            {   // перезапуск этюда
                this.Controls.Clear();
        ...
        {   // кнопка не нажата - фон стандартный
            ...
            if ((userSteps == 1) && (pressedButton.Location.X == 484) && (pressedButton.Location.Y == 236) && (chessboard[6, 4] == 0))
            {
                labelTask27.ForeColor = Color.GreenYellow;
                labelTask27.Text = "Вы успешно решили Этюд №27.";
                MessageBox.Show("Вы успешно решили Этюд №27.", "Обновить Этюд №27", MessageBoxButtons.OK, MessageBoxIcon.Information);
            ...
            {
                labelTask27.ForeColor = Color.OrangeRed;
                labelTask27.Text = "Решить за 1 ход не удалось.";
            }
