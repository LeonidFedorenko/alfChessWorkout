...
namespace alfChessWorkout
{
    ...
    {   // картинка из PNG (все фигуры)
        ...
        {   // 1 - белые, 2 - черные
            ...
            {25,0,0,22,21,23,0,25},
            {26,26,0,23,24,0,26,26},
            {0,0,24,0,0,0,0,0},
            {0,0,26,0,26,26,0,0},
            {0,0,0,0,14,0,0,0},
            {0,0,0,16,0,14,0,0},
            {16,16,16,0,13,16,16,16},
            {15,0,13,12,11,0,0,15},
        };
        ...
        {   // форма для 11 этюда
            ...
            this.Text = " Этюд №17. Мат в один ход. Ход белых.";
            // загрузка фигур для этюда №17
            Init();
        }

        public void showTask17_click(object sender, EventArgs e)
        {
            if (pictureBox1.Visible == true)
            {
                pictureBox1.Visible = false; // скрываю рисунок (с задачей)
                buttonTask17.Text = "   ОБНОВИТЬ       ДАННЫЙ   ЭТЮД";
            }
            else
            {   // перезапуск этюда
                this.Controls.Clear();
        ...
        {   // кнопка не нажата - фон стандартный
            ...
            if ((userSteps == 1) && (pressedButton.Location.X == 236) && (pressedButton.Location.Y == 174) && (chessboard[4, 4] == 0))
            {
                labelTask17.ForeColor = Color.GreenYellow;
                labelTask17.Text = "Вы успешно решили Этюд №17.";
                MessageBox.Show("Вы успешно решили Этюд №17.", "Обновить Этюд №17", MessageBoxButtons.OK, MessageBoxIcon.Information);
            ...
            {
                labelTask17.ForeColor = Color.OrangeRed;
                labelTask17.Text = "Решить за 1 ход не удалось.";
            }
