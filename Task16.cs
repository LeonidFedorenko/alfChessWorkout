...
namespace alfChessWorkout
{
    ...
    {   // картинка из PNG (все фигуры)
        ...
        {   // форма для 16 этюда
            ...
            this.Text = " Этюд №16. Мат в один ход. Ход белых.";
            // загрузка фигур для этюда №10
            Init();
        }

        public void showTask16_click(object sender, EventArgs e)
        {
            if (pictureBox1.Visible == true)
            {
                pictureBox1.Visible = false; // скрываю рисунок (с задачей)
                buttonTask16.Text = "   ОБНОВИТЬ       ДАННЫЙ   ЭТЮД";
            }
            else
            {   // перезапуск этюда
                this.Controls.Clear();
        ...
        {   // кнопка не нажата - фон стандартный
            ...
            if ((userSteps == 1) && (pressedButton.Location.X == 360) && (pressedButton.Location.Y == 484) && (chessboard[2, 0] == 0))
            {
                labelTask16.ForeColor = Color.GreenYellow;
                labelTask16.Text = "Вы успешно решили Этюд №16.";
                MessageBox.Show("Вы успешно решили Этюд №16.", "Обновить Этюд №16", MessageBoxButtons.OK, MessageBoxIcon.Information);
            ...
            {
                labelTask16.ForeColor = Color.OrangeRed;
                labelTask16.Text = "Решить за 1 ход не удалось.";
            }
