...
namespace alfChessWorkout
{
    ...
    {   // картинка из PNG (все фигуры)
        ...
        {   // форма для 1 этюда
            ...
            this.Text = " Этюд №3. Мат в один ход. Ход белых.";
            // загрузка фигур для этюда №3
            Init();
        }

        public void showTask03_click(object sender, EventArgs e)
        {
            if (pictureBox1.Visible == true)
            {
                pictureBox1.Visible = false; // скрываю рисунок (с задачей)
                buttonTask03.Text = "   ОБНОВИТЬ       ДАННЫЙ   ЭТЮД";
            }
            else
            {   // перезапуск этюда
                this.Controls.Clear();
        ...
        {   // кнопка не нажата - фон стандартный
            ...
            if ((userSteps == 1) && (pressedButton.Location.X == 174) && (pressedButton.Location.Y == 422) && (chessboard[1, 7] == 0))
            {
                labelTask03.ForeColor = Color.GreenYellow;
                labelTask03.Text = "Вы успешно решили Этюд №3.";
                MessageBox.Show("Вы успешно решили Этюд №3.", "Обновить Этюд №3", MessageBoxButtons.OK, MessageBoxIcon.Information);
            ...
            {
                labelTask03.ForeColor = Color.OrangeRed;
                labelTask03.Text = "Решить за 1 ход не удалось.";
            }
