...
namespace alfChessWorkout
{
    ...
    {   // картинка из PNG (все фигуры)
        ...
        {   // форма для 11 этюда
            ...
            this.Text = " Этюд №23. Мат в один ход. Ход белых.";
            // загрузка фигур для этюда №23
            Init();
        }

        public void showTask23_click(object sender, EventArgs e)
        {
            if (pictureBox1.Visible == true)
            {
                pictureBox1.Visible = false; // скрываю рисунок (с задачей)
                buttonTask23.Text = "   ОБНОВИТЬ       ДАННЫЙ   ЭТЮД";
            }
            else
            {   // перезапуск этюда
                this.Controls.Clear();
        ...
        {   // кнопка не нажата - фон стандартный
            ...
            if ((userSteps == 1) && (pressedButton.Location.X == 484) && (pressedButton.Location.Y == 484) && (chessboard[0, 0] == 0))
            {
                labelTask23.ForeColor = Color.GreenYellow;
                labelTask23.Text = "Вы успешно решили Этюд №23.";
                MessageBox.Show("Вы успешно решили Этюд №23.", "Обновить Этюд №23", MessageBoxButtons.OK, MessageBoxIcon.Information);
            ...
            {
                labelTask23.ForeColor = Color.OrangeRed;
                labelTask23.Text = "Решить за 1 ход не удалось.";
            }
