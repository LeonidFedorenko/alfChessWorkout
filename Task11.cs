...
namespace alfChessWorkout
{
    ...
    {   // картинка из PNG (все фигуры)
        ...
        {   // 1 - белые, 2 - черные
            ...
            {0,0,0,25,0,0,25,0},
            {0,26,21,0,24,26,0,22},
            {26,24,23,0,26,0,26,0},
            {15,0,0,0,0,0,0,26},
            {0,0,0,0,0,0,0,0},
            {0,16,0,13,0,14,0,16},
            {0,13,16,0,11,16,16,0},
            {0,0,0,0,15,0,12,0},
        };
        ...
        {   // форма для 11 этюда
            ...
            chessAllSprites = new Bitmap(alfChessWorkout.Properties.Resources.chess_children); // файл chess_children.png
            // указываю заголовок окна
            this.Text = " Этюд №11. Мат в один ход. Ход белых.";
            // загрузка фигур для этюда №11
            Init();
        }

        public void showTask11_click(object sender, EventArgs e)
        {
            if (pictureBox1.Visible == true)
            {
                pictureBox1.Visible = false; // скрываю рисунок (с задачей)
                buttonTask11.Text = "   ОБНОВИТЬ       ДАННЫЙ   ЭТЮД";
            }
            else
            {   // перезапуск этюда
                this.Controls.Clear();
        ...
        {   // расстановка фигур по умолчанию (для перезапуска партии)
            ...
            if (whitePlayer == 0) // и загружаю из ресурсов проекта)
            {
                ...
                {   // 1 - белые, 2 - черные
                    // 2 - ферзь, 1 - король, 5 - ладья, 4 - конь, 3 - слон, 6 - пешка 
                    {0,0,0,25,0,0,25,0},
                    {0,26,22,0,24,26,0,21},
                    {26,24,23,0,26,0,26,0},
                    {15,0,0,0,0,0,0,26},
                    {0,0,0,0,0,0,0,0},
                    {0,16,0,13,0,14,0,16},
                    {0,13,16,0,12,16,16,0},
                    {0,0,0,0,15,0,11,0},
                };
            }
        ...
        {   // кнопка не нажата - фон стандартный
            ...
            if ((userSteps == 1) && (pressedButton.Location.X == 484) && (pressedButton.Location.Y == 236))
            {
                labelTask11.ForeColor = Color.GreenYellow;
                labelTask11.Text = "Вы успешно решили Этюд №11.";
                MessageBox.Show("Вы успешно решили Этюд №11.", "Обновить Этюд №11", MessageBoxButtons.OK, MessageBoxIcon.Information);
            ...
            {
                labelTask11.ForeColor = Color.OrangeRed;
                labelTask11.Text = "Решить за 1 ход не удалось.";
            }
