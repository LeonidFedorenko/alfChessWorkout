...
namespace alfChessWorkout
{
    ...
    {   // картинка из PNG (все фигуры)
        ...
        public int whitePlayer = 1; // ходят белые фигуры 
        // количество шагов 
        ...
        {   // форма для 19 этюда
            ...
            chessAllSprites = new Bitmap(alfChessWorkout.Properties.Resources.chess); // файл chess.png
            // указываю заголовок окна
            this.Text = " Этюд №19. Мат в один ход. Ход черных.";
            // загрузка фигур для этюда №19
            Init();
        }

        public void showTask19_click(object sender, EventArgs e)
        {
            if (pictureBox1.Visible == true)
            {
                pictureBox1.Visible = false; // скрываю рисунок (с задачей)
                buttonTask19.Text = "   ОБНОВИТЬ       ДАННЫЙ   ЭТЮД";
            }
            else
            {   // перезапуск этюда
                this.Controls.Clear();
        ...
        {   // расстановка фигур по умолчанию (для перезапуска партии)
            whitePlayer = 1;
            userSteps = 0;
            if (whitePlayer == 1) // и загружаю из ресурсов проекта)
            {
            ...
            currentPlayer = 2; // текущий игрок по умолчанию (черный)
        ...
        {   // создаю доску для игры (8 x 8)
            for (int iChess = 0; iChess < 8; iChess++)
            {
                ...
                {   // массив возможных ходов для каждой фигуры
                    ...
                    buttonF.BackColor = Color.FromArgb(181,136,99); // темный фон кнопки-фигуры
                    if ((iChess + jChess) % 2 == 0)
                    {
                        buttonF.BackColor = Color.FromArgb(240, 217, 181); // светлый фон кнопки-фигуры    
                    }
        ...
        {   // кнопка не нажата - фон стандартный
            ...
            {
                if (previousButton.BackColor == Color.FromArgb(240, 217, 181))
                {   // был светлый - оставляю
                    previousButton.BackColor = Color.FromArgb(240, 217, 181);
                }
                else if (previousButton.BackColor == Color.FromArgb(181, 136, 99))
                {   // был тймный - оставляю
                    previousButton.BackColor = Color.FromArgb(181, 136, 99);
                }
            }
            ...
            if ((userSteps == 1) && (pressedButton.Location.X == 422) && (pressedButton.Location.Y == 422) && (chessboard[5, 6] == 0))
            {
                labelTask19.ForeColor = Color.GreenYellow;
                labelTask19.Text = "Вы успешно решили Этюд №19.";
                MessageBox.Show("Вы успешно решили Этюд №19.", "Обновить Этюд №19", MessageBoxButtons.OK, MessageBoxIcon.Information);
            ...
            {
                labelTask19.ForeColor = Color.OrangeRed;
                labelTask19.Text = "Решить за 1 ход не удалось.";
            }
        ...
        {   // убираю вспомогательные шаги-подсказки
            for (int iStep = 0; iStep < 8; iStep++)
            {
                for (int jStep = 0; jStep < 8; jStep++)
                {
                    ...
                    {
                        virtualButtons[iStep, jStep].BackColor = Color.FromArgb(240, 217, 181);
                    }
                    else if ((iStep % 2 == 1) && ((jStep == 0) || (jStep == 2) || (jStep == 4) || (jStep == 6) || (jStep == 8)))
                    {
                        virtualButtons[iStep, jStep].BackColor = Color.FromArgb(181, 136, 99);
                    }
