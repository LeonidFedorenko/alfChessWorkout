...
namespace alfChessWorkout
{
    ...
    {   // картинка из PNG (все фигуры)
        ...
        {   // форма для 12 этюда
            ...
            this.Text = " Этюд №32. Мат в один ход. Ход белых.";
            // загрузка фигур для этюда №32
            Init();
        }

        public void showTask32_click(object sender, EventArgs e)
        {
            if (pictureBox1.Visible == true)
            {
                pictureBox1.Visible = false; // скрываю рисунок (с задачей)
                buttonTask32.Text = "   ОБНОВИТЬ       ДАННЫЙ   ЭТЮД";
            }
            else
            {   // перезапуск этюда
                this.Controls.Clear();
        ...
        {   // кнопка не нажата - фон стандартный
            ...
            if ((userSteps == 1) && (pressedButton.Location.X == 360) && (pressedButton.Location.Y == 50) && (chessboard[7,2]==0))
            {
                labelTask32.ForeColor = Color.GreenYellow;
                labelTask32.Text = "Вы успешно решили Этюд №32.";
                MessageBox.Show("Вы успешно решили Этюд №32.", "Обновить Этюд №32", MessageBoxButtons.OK, MessageBoxIcon.Information);
            ...
            {
                labelTask32.ForeColor = Color.WhiteSmoke;
                labelTask32.Text = "Решить за 1 ход не удалось.";
            }
