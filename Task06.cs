...
namespace alfChessWorkout
{
    ...
    {   // картинка из PNG (все фигуры)
        ...
        {   // форма для 1 этюда
            ...
            this.Text = " Этюд №6. Мат в один ход. Ход черных.";
            // загрузка фигур для этюда №6
            Init();
        }

        public void showTask06_click(object sender, EventArgs e)
        {
            if (pictureBox1.Visible == true)
            {
                pictureBox1.Visible = false; // скрываю рисунок (с задачей)
                buttonTask06.Text = "   ОБНОВИТЬ       ДАННЫЙ   ЭТЮД";
            }
            else
            {   // перезапуск этюда
                this.Controls.Clear();
        ...
        {   // кнопка не нажата - фон стандартный
            ...
            if ((userSteps == 1) && (pressedButton.Location.X == 298) && (pressedButton.Location.Y == 422) && (chessboard[4, 5] == 0))
            {
                labelTask06.ForeColor = Color.GreenYellow;
                labelTask06.Text = "Вы успешно решили Этюд №6.";
                MessageBox.Show("Вы успешно решили Этюд №6.", "Обновить Этюд №6", MessageBoxButtons.OK, MessageBoxIcon.Information);
            ...
            {
                labelTask06.ForeColor = Color.OrangeRed;
                labelTask06.Text = "Решить за 1 ход не удалось.";
            }
