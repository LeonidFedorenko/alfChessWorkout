...
namespace alfChessWorkout
{
    ...
    {   // картинка из PNG (все фигуры)
        ...
        {   // форма для 1 этюда
            ...
            this.Text = " Этюд №34. Мат в один ход. Ход белых.";
            // загрузка фигур для этюда №34
            Init();
        }

        public void showTask34_click(object sender, EventArgs e)
        {
            if (pictureBox1.Visible == true)
            {
                pictureBox1.Visible = false; // скрываю рисунок (с задачей)
                buttonTask34.Text = "   ОБНОВИТЬ       ДАННЫЙ   ЭТЮД";
            }
            else
            {   // перезапуск этюда
                this.Controls.Clear();
        ...
        {   // кнопка не нажата - фон стандартный
            ...
            if ((userSteps == 1) && (pressedButton.Location.X == 112) && (pressedButton.Location.Y == 422) && (chessboard[7, 2] == 0))
            {
                labelTask34.ForeColor = Color.GreenYellow;
                labelTask34.Text = "Вы успешно решили Этюд №34.";
                MessageBox.Show("Вы успешно решили Этюд №34.", "Обновить Этюд №34", MessageBoxButtons.OK, MessageBoxIcon.Information);
            ...
            {
                labelTask34.ForeColor = Color.OrangeRed;
                labelTask34.Text = "Решить за 1 ход не удалось.";
            }
