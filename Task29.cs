...
namespace alfChessWorkout
{
    ...
    {   // картинка из PNG (все фигуры)
        ...
        {   // форма для 29 этюда
            ...
            this.Text = " Этюд №29. Мат в один ход. Ход белых.";
            // загрузка фигур для этюда №29
            Init();
        }

        public void showTask29_click(object sender, EventArgs e)
        {
            if (pictureBox1.Visible == true)
            {
                pictureBox1.Visible = false; // скрываю рисунок (с задачей)
                buttonTask29.Text = "   ОБНОВИТЬ       ДАННЫЙ   ЭТЮД";
            }
            else
            {   // перезапуск этюда
                this.Controls.Clear();
        ...
        {   // кнопка не нажата - фон стандартный
            ...
            if ((userSteps == 1) && (pressedButton.Location.X == 422) && (pressedButton.Location.Y == 236) && (chessboard[3, 0] == 0))
            {
                labelTask29.ForeColor = Color.GreenYellow;
                labelTask29.Text = "Вы успешно решили Этюд №29.";
                MessageBox.Show("Вы успешно решили Этюд №29.", "Обновить Этюд №29", MessageBoxButtons.OK, MessageBoxIcon.Information);
            ...
            {
                labelTask29.ForeColor = Color.OrangeRed;
                labelTask29.Text = "Решить за 1 ход не удалось.";
            }
