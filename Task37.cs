...
namespace alfChessWorkout
{
    ...
    {   // картинка из PNG (все фигуры)
        ...
        {   // форма для 37 этюда
            ...
            this.Text = " Этюд №37. Мат в один ход. Ход белых.";
            // загрузка фигур для этюда №37
            Init();
        }

        public void showTask37_click(object sender, EventArgs e)
        {
            if (pictureBox1.Visible == true)
            {
                pictureBox1.Visible = false; // скрываю рисунок (с задачей)
                buttonTask37.Text = "   ОБНОВИТЬ       ДАННЫЙ   ЭТЮД";
            }
            else
            {   // перезапуск этюда
                this.Controls.Clear();
        ...
        {   // кнопка не нажата - фон стандартный
            ...
            if ((userSteps == 1) && (pressedButton.Location.X == 484) && (pressedButton.Location.Y == 50) && (chessboard[7, 5] == 0))
            {
                labelTask37.ForeColor = Color.GreenYellow;
                labelTask37.Text = "Вы успешно решили Этюд №37.";
                MessageBox.Show("Вы успешно решили Этюд №37.", "Обновить Этюд №37", MessageBoxButtons.OK, MessageBoxIcon.Information);
            ...
            {
                labelTask37.ForeColor = Color.OrangeRed;
                labelTask37.Text = "Решить за 1 ход не удалось.";
            }
