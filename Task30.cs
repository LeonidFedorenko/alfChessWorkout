...
namespace alfChessWorkout
{
    ...
    {   // картинка из PNG (все фигуры)
        ...
        {   // форма для 30 этюда
            ...
            this.Text = " Этюд №30. Мат в один ход. Ход белых.";
            // загрузка фигур для этюда №30
            Init();
        }

        public void showTask30_click(object sender, EventArgs e)
        {
            if (pictureBox1.Visible == true)
            {
                pictureBox1.Visible = false; // скрываю рисунок (с задачей)
                buttonTask30.Text = "   ОБНОВИТЬ       ДАННЫЙ   ЭТЮД";
            }
            else
            {   // перезапуск этюда
                this.Controls.Clear();
        ...
        {   // кнопка не нажата - фон стандартный
            ...
            if ((userSteps == 1) && (pressedButton.Location.X == 360) && (pressedButton.Location.Y == 112) && (chessboard[4, 3] == 0))
            {
                labelTask30.ForeColor = Color.GreenYellow;
                labelTask30.Text = "Вы успешно решили Этюд №30.";
                MessageBox.Show("Вы успешно решили Этюд №30.", "Обновить Этюд №30", MessageBoxButtons.OK, MessageBoxIcon.Information);
            ...
            {
                labelTask30.ForeColor = Color.OrangeRed;
                labelTask30.Text = "Решить за 1 ход не удалось.";
            }
