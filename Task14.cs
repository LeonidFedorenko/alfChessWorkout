...
namespace alfChessWorkout
{
    ...
    {   // картинка из PNG (все фигуры)
        ...
        {   // форма для 14 этюда
            ...
            this.Text = " Этюд №14. Мат в один ход. Ход белых.";
            // загрузка фигур для этюда №14
            Init();
        }

        public void showTask14_click(object sender, EventArgs e)
        {
            if (pictureBox1.Visible == true)
            {
                pictureBox1.Visible = false; // скрываю рисунок (с задачей)
                buttonTask14.Text = "   ОБНОВИТЬ       ДАННЫЙ   ЭТЮД";
            }
            else
            {   // перезапуск этюда
                this.Controls.Clear();
        ...
        {   // кнопка не нажата - фон стандартный
            ...
            if ((userSteps == 1) && (pressedButton.Location.X == 422) && (pressedButton.Location.Y == 236) && (chessboard[4, 6] == 0))
            {
                labelTask14.ForeColor = Color.GreenYellow;
                labelTask14.Text = "Вы успешно решили Этюд №14.";
                MessageBox.Show("Вы успешно решили Этюд №14.", "Обновить Этюд №14", MessageBoxButtons.OK, MessageBoxIcon.Information);
            ...
            {
                labelTask14.ForeColor = Color.OrangeRed;
                labelTask14.Text = "Решить за 1 ход не удалось.";
            }
