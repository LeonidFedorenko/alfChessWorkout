...
namespace alfChessWorkout
{
    ...
    {   // картинка из PNG (все фигуры)
        ...
        {   // форма для 21 этюда
            ...
            this.Text = " Этюд №21. Мат в один ход. Ход белых.";
            // загрузка фигур для этюда №21
            Init();
        }

        public void showTask21_click(object sender, EventArgs e)
        {
            if (pictureBox1.Visible == true)
            {
                pictureBox1.Visible = false; // скрываю рисунок (с задачей)
                buttonTask21.Text = "   ОБНОВИТЬ       ДАННЫЙ   ЭТЮД";
            }
            else
            {   // перезапуск этюда
                this.Controls.Clear();
        ...
        {   // кнопка не нажата - фон стандартный
            ...
            if ((userSteps == 1) && (pressedButton.Location.X == 174) && (pressedButton.Location.Y == 112) && (chessboard[6, 7] == 0))
            {
                labelTask21.ForeColor = Color.GreenYellow;
                labelTask21.Text = "Вы успешно решили Этюд №21.";
                MessageBox.Show("Вы успешно решили Этюд №21.", "Обновить Этюд №21", MessageBoxButtons.OK, MessageBoxIcon.Information);
            ...
            {
                labelTask21.ForeColor = Color.OrangeRed;
                labelTask21.Text = "Решить за 1 ход не удалось.";
            }
