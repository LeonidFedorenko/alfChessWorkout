...
namespace alfChessWorkout
{
    ...
    {   // картинка из PNG (все фигуры)
        ...
        {   // форма для 25 этюда
            ...
            this.Text = " Этюд №25. Мат в один ход. Ход белых.";
            // загрузка фигур для этюда №25
            Init();
        }

        public void showTask25_click(object sender, EventArgs e)
        {
            if (pictureBox1.Visible == true)
            {
                pictureBox1.Visible = false; // скрываю рисунок (с задачей)
                buttonTask25.Text = "   ОБНОВИТЬ       ДАННЫЙ   ЭТЮД";
            }
            else
            {   // перезапуск этюда
                this.Controls.Clear();
        ...
        {   // кнопка не нажата - фон стандартный
            ...
            if ((userSteps == 1) && (pressedButton.Location.X == 50) && (pressedButton.Location.Y == 112) && (chessboard[5, 6] == 0))
            {
                labelTask25.ForeColor = Color.GreenYellow;
                labelTask25.Text = "Вы успешно решили Этюд №25.";
                MessageBox.Show("Вы успешно решили Этюд №25.", "Обновить Этюд №25", MessageBoxButtons.OK, MessageBoxIcon.Information);
            ...
            {
                labelTask25.ForeColor = Color.OrangeRed;
                labelTask25.Text = "Решить за 1 ход не удалось.";
            }
