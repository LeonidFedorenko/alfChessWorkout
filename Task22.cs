...
namespace alfChessWorkout
{
    ...
    {   // картинка из PNG (все фигуры)
        ...
        {   // форма для 22 этюда
            ...
            this.Text = " Этюд №22. Мат в один ход. Ход белых.";
            // загрузка фигур для этюда №10
            Init();
        }

        public void showTask22_click(object sender, EventArgs e)
        {
            if (pictureBox1.Visible == true)
            {
                pictureBox1.Visible = false; // скрываю рисунок (с задачей)
                buttonTask22.Text = "   ОБНОВИТЬ       ДАННЫЙ   ЭТЮД";
            }
            else
            {   // перезапуск этюда
                this.Controls.Clear();
        ...
        {   // кнопка не нажата - фон стандартный
            ...
            if ((userSteps == 1) && (pressedButton.Location.X == 298) && (pressedButton.Location.Y == 50) && (chessboard[7, 4] == 0))
            {
                labelTask22.ForeColor = Color.GreenYellow;
                labelTask22.Text = "Вы успешно решили Этюд №22.";
                MessageBox.Show("Вы успешно решили Этюд №22.", "Обновить Этюд №22", MessageBoxButtons.OK, MessageBoxIcon.Information);
            ...
            {
                labelTask22.ForeColor = Color.OrangeRed;
                labelTask22.Text = "Решить за 1 ход не удалось.";
            }
