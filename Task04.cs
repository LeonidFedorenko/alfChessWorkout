...
namespace alfChessWorkout
{
    ...
    {   // картинка из PNG (все фигуры)
        ...
        {   // форма для 1 этюда
            ...
            this.Text = " Этюд №4. Мат в один ход. Ход белых.";
            // загрузка фигур для этюда №1
            Init();
        }

        public void showTask04_click(object sender, EventArgs e)
        {
            if (pictureBox1.Visible == true)
            {
                pictureBox1.Visible = false; // скрываю рисунок (с задачей)
                buttonTask04.Text = "   ОБНОВИТЬ       ДАННЫЙ   ЭТЮД";
            }
            else
            {   // перезапуск этюда
                this.Controls.Clear();
        ...
        {   // кнопка не нажата - фон стандартный
            ...
            if ((userSteps == 1) && (pressedButton.Location.X == 422) && (pressedButton.Location.Y == 236) && (chessboard[5, 4] == 0))
            {
                labelTask04.ForeColor = Color.GreenYellow;
                labelTask04.Text = "Вы успешно решили Этюд №4.";
                MessageBox.Show("Вы успешно решили Этюд №4.", "Обновить Этюд №4", MessageBoxButtons.OK, MessageBoxIcon.Information);
            ...
            {
                labelTask04.ForeColor = Color.OrangeRed;
                labelTask04.Text = "Решить за 1 ход не удалось.";
            }
