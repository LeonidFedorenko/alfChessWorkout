...
namespace alfChessWorkout
{
    ...
    {   // картинка из PNG (все фигуры)
        ...
        {   // форма для 1 этюда
            ...
            this.Text = " Этюд №48. Мат в один ход. Ход черных.";
            // загрузка фигур для этюда №1
            Init();
        }

        public void showTask48_click(object sender, EventArgs e)
        {
            if (pictureBox1.Visible == true)
            {
                pictureBox1.Visible = false; // скрываю рисунок (с задачей)
                buttonTask48.Text = "   ОБНОВИТЬ       ДАННЫЙ   ЭТЮД";
            }
            else
            {   // перезапуск этюда
                this.Controls.Clear();
        ...
        {   // расстановка фигур по умолчанию (для перезапуска партии)
            ...
            currentPlayer = 2; // текущий игрок по умолчанию
        ...
        {   // кнопка не нажата - фон стандартный
            ...
            if ((userSteps == 1) && (pressedButton.Location.X == 298) && (pressedButton.Location.Y == 484) && (chessboard[4, 7] == 0))
            {
                labelTask48.ForeColor = Color.GreenYellow;
                labelTask48.Text = "Вы успешно решили Этюд №48.";
                MessageBox.Show("Вы успешно решили Этюд №48.", "Обновить Этюд №48", MessageBoxButtons.OK, MessageBoxIcon.Information);
            ...
            {
                labelTask48.ForeColor = Color.OrangeRed;
                labelTask48.Text = "Решить за 1 ход не удалось.";
            }
