...
namespace alfChessWorkout
{
    ...
    {   // картинка из PNG (все фигуры)
        ...
        {   // 1 - белые, 2 - черные
            ...
            {25,0,0,0,22,23,0,25},
            {26,26,26,23,21,0,26,0},
            {0,0,24,26,0,0,24,0},
            {0,0,0,0,26,0,26,0},
            {0,0,13,16,16,0,0,0},
            {0,0,16,0,0,0,0,0},
            {16,16,0,0,0,16,16,16},
            {15,14,13,0,0,15,11,0},
        };
        ...
        public Button ISButton; // контроль за фугурой, которой был ход

        public Task44()
        {   // форма для 44 этюда
            InitializeComponent();
            // загрузка фигур из одного ресурса - спрайтами
            chessAllSprites = new Bitmap(alfChessWorkout.Properties.Resources.chess_two); // файл chess_two.png
            // указываю заголовок окна
            this.Text = " Этюд №44. Мат в один ход. Ход белых.";
            // загрузка фигур для этюда №44
            Init();
        }

        public void showTask44_click(object sender, EventArgs e)
        {
            if (pictureBox1.Visible == true)
            {
                pictureBox1.Visible = false; // скрываю рисунок (с задачей)
                buttonTask44.Text = "   ОБНОВИТЬ       ДАННЫЙ   ЭТЮД";
            }
            else
            {   // перезапуск этюда
                this.Controls.Clear();
        ...
        {   // создаю доску для игры (8 x 8)
            for (int iChess = 0; iChess < 8; iChess++)
            {
                ...
                {   // массив возможных ходов для каждой фигуры
                    ...
                    buttonF.BackColor = Color.FromArgb(142,36,38); // темный фон кнопки-фигуры
                    if ((iChess + jChess) % 2 == 0)
                    {
                        buttonF.BackColor = Color.FromArgb(255, 251, 219); // светлый фон кнопки-фигуры    
                    }
        ...
        {   // кнопка не нажата - фон стандартный
            ...
            {
                if (previousButton.BackColor == Color.FromArgb(255, 251, 219))
                {   // был светлый - оставляю
                    previousButton.BackColor = Color.FromArgb(255, 251, 219);
                }
                else if (previousButton.BackColor == Color.FromArgb(142, 36, 38))
                {   // был тёмный - оставляю
                    previousButton.BackColor = Color.FromArgb(142, 36, 38);
                }
            }
            ...
            ISButton = pressedButtonTemp;
            //Button ISButton = pressedButtonTemp; // какой фигурой сделан ход 
            ...
            // проверяю какой фигурой был ход, из начального массива
            if ((userSteps == 1) && (pressedButton.Location.X == 422) && (pressedButton.Location.Y == 236) && (chessboard[2,7]==0))
            {
                labelTask44.ForeColor = Color.GreenYellow;
                labelTask44.Text = "Вы успешно решили Этюд №44.";
                MessageBox.Show("Вы успешно решили Этюд №44.", "Обновить Этюд №44", MessageBoxButtons.OK, MessageBoxIcon.Information);
                this.Controls.Clear(); // чистка всех конролов
                InitializeComponent(); // загрузка по новой
                // загрузка фигур из ресурса - спрайтами
                chessAllSprites = new Bitmap(alfChessWorkout.Properties.Resources.chess_two); // файл chess_two.png
                Init(); // расстановка фигур для этюда
            }
            else if (userSteps > 1)
            {
                labelTask44.ForeColor = Color.WhiteSmoke;
                labelTask44.Text = "Решить за 1 ход не удалось.";
            }
        ...
        {   // убираю вспомогательные шаги-подсказки
            for (int iStep = 0; iStep < 8; iStep++)
            {
                for (int jStep = 0; jStep < 8; jStep++)
                {
                    ...
                    {
                        virtualButtons[iStep, jStep].BackColor = Color.FromArgb(255, 251, 219);
                    }
                    else if ((iStep % 2 == 1) && ((jStep == 0) || (jStep == 2) || (jStep == 4) || (jStep == 6) || (jStep == 8)))
                    {
                        virtualButtons[iStep, jStep].BackColor = Color.FromArgb(142, 36, 38);
                    }
