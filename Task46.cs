...
namespace alfChessWorkout
{
    ...
    {   // картинка из PNG (все фигуры)
        ...
        {   // форма для 46 этюда
            ...
            this.Text = " Этюд №46. Мат в один ход. Ход белых.";
            // загрузка фигур для этюда №46
            Init();
        }

        public void showTask46_click(object sender, EventArgs e)
        {
            if (pictureBox1.Visible == true)
            {
                pictureBox1.Visible = false; // скрываю рисунок (с задачей)
                buttonTask46.Text = "   ОБНОВИТЬ       ДАННЫЙ   ЭТЮД";
            }
            else
            {   // перезапуск этюда
                this.Controls.Clear();
        ...
        {   // кнопка не нажата - фон стандартный
            ...
            if ((userSteps == 1) && (pressedButton.Location.X == 422) && (pressedButton.Location.Y == 174) && (chessboard[3, 4] == 0))
            {
                labelTask46.ForeColor = Color.GreenYellow;
                labelTask46.Text = "Вы успешно решили Этюд №46.";
                MessageBox.Show("Вы успешно решили Этюд №46.", "Обновить Этюд №46", MessageBoxButtons.OK, MessageBoxIcon.Information);
            ...
            {
                labelTask46.ForeColor = Color.OrangeRed;
                labelTask46.Text = "Решить за 1 ход не удалось.";
            }
